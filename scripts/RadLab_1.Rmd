...
studies_all_relevant <- studies_all[1:15,]

# Only selecting the relevant vignettes from the pilot
# The following was NEVER used in any study at all
values_to_remove <- c(4, 8, 12, 17, 20, 21)
v.means_pilot <- v.means_pilot %>% filter(!order %in% values_to_remove)
...
########## Order: Pilot ##########
o_pilot <- studies_all_relevant[
  !is.na(studies_all_relevant[,2]), c(1,2)]

########## Order: Study1 ##########
o_study1 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  1)

########## Order: Study2 ##########
o_study2 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  2)

########## Order: Study3 ##########
o_study3 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  3)

########## Order: Study4 ##########
o_study4 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  4)

########## Order: Study5 ##########
o_study5 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  5)

########## Order: Study6 ##########
o_study6 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  6)

########## Order: Study7 ##########
o_study7 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  7)

########## Order: Study8 ##########
o_study8 <- RadLab::merge_with_pilot(
  studies_all,
  v.means_pilot,
  8)
```
...
study_1_means_plot <- d_study1[,1:60] %>%
  RadLab::graph_means(
    name_data = o_study1,
    n_vignettes = 15)
```
...
save(d_study1, file = "data_working/d_study1.Rdata")
```
...
save(d_study2, file = "data_working/d_study2.Rdata")
```
...
save(d_study3, file = "data_working/d_study3.Rdata")
```
...
save(d_study4, file = "data_working/d_study4.Rdata")
```
...
save(d_study5, file = "data_working/d_study5.Rdata")
```
...
save(d_study6, file = "data_working/d_study6.Rdata")
```
...
d_study7_0 <- haven::read_spss(
  'data_cleaned_allard/22-23_Radlab_Jordan.sav')
```
...
save(d_study7, file = "data_working/d_study7.Rdata")
```
...
d_study8_0 <- haven::read_spss(
  'data_cleaned_allard/23-24_Radlab_DeRee.sav')
```
...
save(d_study8, file = "data_working/d_study8.Rdata")
```
...
save(data_combined, file = "data_working/d_combined.Rdata")
```
