...
# Define the original variables you want to use for evaluation:
feature_columns_original = ['ina', 'na', 'nna', 'enna']  # add 'time' here if you want: ['ina', 'na', 'nna', 'enna', 'time']

grouped = df.groupby("ID")
participant_ids = list(grouped.groups.keys())  # Same ordering as used for time_series_data and labels
...
for exp in experiments:
    ...
    for pid in exp_ids:
        # Extract the participant's original data, sorted by time
        p_df = df[df['ID'] == pid].sort_values('time')
        
        # Get just the original compositional variables (and time if included)
        p_data = p_df[feature_columns_original].values
        
        # Remove rows with NaNs if any
        nan_mask = np.isnan(p_data).any(axis=1)
        if np.any(nan_mask):
            p_data = p_data[~nan_mask]

        if len(p_data) == 0:
            # Skip participants with no valid data
            continue
        
        participant_series.append(p_data)
        
        # Retrieve the cluster label for this participant
        p_index = id_to_index[pid]
        exp_labels.append(labels[p_index])
    
    if len(participant_series) < 2:
    ...
    # (n_participants, min_len * n_features)
    data_matrix = np.array([t.flatten() for t in truncated_series])
    exp_labels = np.array(exp_labels)
    
    # Compute Silhouette and Davies-Bouldin Scores in this original space
    try:
...
savgol_window = 2  # Window length for Savitzky-Golay filter
